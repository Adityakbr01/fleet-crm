...
const VehicleTravelList = () => {
  ...
  const {
    data: vehicleTravelPushDatesResponse,
    isLoading: isPushDatesLoading,
    refetch: refetchPushDates,
  } = useQuery({
    queryKey: ["vehicle-travel-push-dates"],
    queryFn: async () => {
      const token = Cookies.get("token");
      const response = await axios.get(
        `${BASE_URL}/api/getVehicleTravelPushDate`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
  ...
  const categorizedDates = useMemo(() => {
    const data =
      vehicleTravelPushDatesResponse?.data || vehicleTravelPushDatesResponse;
    if (!data) return {};
        ...
        if (Array.isArray(val)) {
          ...
          val.forEach((item) => {
            const dateStr =
              typeof item === "string"
                ? item
                : item?.vehicle_travel_push_date ||
                  item?.travel_push_date ||
                  item?.push_date ||
                  item?.date;
    ...
    return { "Vehicle Travel Push Dates": set };
  }, [vehicleTravelPushDatesResponse]);
          ...
          <Popover open={calendarOpen} onOpenChange={setCalendarOpen}>
            ...
            <PopoverContent align="end" className="w-auto p-4 max-w-[95vw] overflow-x-auto">
              <div className="flex items-center justify-between pb-2 mb-3 border-b">
                <p className="text-xs font-semibold text-gray-800">Vehicle Travel Push Dates</p>
