...
const DriverPerformanceList = () => {
  ...
  const {
    data: driverPushDatesResponse,
    isLoading: isPushDatesLoading,
    refetch: refetchPushDates,
  } = useQuery({
    queryKey: ["driver-push-dates"],
    queryFn: async () => {
      const token = Cookies.get("token");
      const response = await axios.get(`${BASE_URL}/api/getDriverPushDate`, {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
        },
  ...
  const categorizedDates = useMemo(() => {
    const data = driverPushDatesResponse?.data || driverPushDatesResponse;
    if (!data) return {};
    ...
    return { "Driver Push Dates": set };
  }, [driverPushDatesResponse]);
          ...
          <Popover open={calendarOpen} onOpenChange={setCalendarOpen}>
            ...
            <PopoverContent align="end" className="w-auto p-4 max-w-[95vw] overflow-x-auto">
              <div className="flex items-center justify-between pb-2 mb-3 border-b">
                <p className="text-xs font-semibold text-gray-800">Driver Push Dates</p>
